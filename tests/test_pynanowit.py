...
"""
    ...
    Tests the PyNanoWit application.
...
import tempfile
import pytest
from pynanowit import pynanowit


@pytest.fixture
def client(request):
    db_fd, pynanowit.app.config['DATABASE'] = tempfile.mkstemp()
    client = pynanowit.app.test_client()
    with pynanowit.app.app_context():
        pynanowit.init_db()

    def teardown():
        """Get rid of the database again after each test."""
        os.close(db_fd)
        os.unlink(pynanowit.app.config['DATABASE'])
